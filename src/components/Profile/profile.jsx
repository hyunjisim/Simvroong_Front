...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const fetchPartnerStatus = async () => { 
      try{
        ...
        const response = await axios.get("http://192.168.163.8:8080/partnership/getpartner", {
          headers: {
    ...
    const fetchNickname = async () => {
        try {
            const token = sessionStorage.getItem("authToken");
            const response = await axios.get("http://192.168.163.8:8080/profile/getNickname", {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
  ...
  const goVroongMoney = () => navigate("/money"); // 부릉머니 페이지로 이동
